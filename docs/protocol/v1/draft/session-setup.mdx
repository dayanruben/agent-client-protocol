...
If the Agent requires authentication, `session/new` may fail with an `auth_required` error until the Client completes the [authentication flow](/protocol/v1/draft/authentication).
...
The Agent **MUST** replay the entire conversation to the Client in the form of `session/update` notifications (like `session/prompt`). Live-only `notice` updates are not conversation history and **SHOULD NOT** be replayed. If the underlying condition is still relevant after loading finishes, the Agent may emit a new notice as a new live event.
...
[`session/cancel`](/protocol/v1/prompt-turn#cancellation) had been called, then free the
