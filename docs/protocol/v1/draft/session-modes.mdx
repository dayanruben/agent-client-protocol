...
<Note>
  You can now use [Session Config
  Options](/protocol/v1/draft/session-config-options). Dedicated session mode
  methods will be removed in a future version of the protocol. Until then, you
  can offer both to clients for backwards compatibility.
</Note>
...
During [Session Setup](/protocol/v1/draft/session-setup) the Agent **MAY** return a list of modes it can operate in and the currently active mode:
...
Typically, Clients display the available modes to the user and allow them to change the current one, which they can do by calling the [`session/set_mode`](/protocol/v1/draft/schema#session%2Fset-mode) method.
...
<Card
  icon="shield-check"
  horizontal
  href="/protocol/v1/draft/tool-calls#requesting-permission"
>
