...
The Initialization phase allows [Clients](/protocol/v1/draft/overview#client) and [Agents](/protocol/v1/draft/overview#agent) to negotiate protocol versions, capabilities, and authentication methods.
...
Implementations can also [advertise custom capabilities](/protocol/v1/draft/extensibility#advertising-custom-capabilities) using the `_meta` field to indicate support for protocol extensions.
...
<Card icon="terminal" horizontal href="/protocol/v1/draft/authentication">
...
<Card icon="file" horizontal href="/protocol/v1/draft/file-system">
...
<Card icon="message-question" horizontal href="/protocol/v1/draft/elicitation">
...
<ParamField path="session.compaction" type="CompactionCapabilities Object">
...
<ParamField path="session.notices" type="NoticeCapabilities Object">
  The Client can present live advisory `notice` session updates to the user.
  This field is optional and nullable. Omitting `session` or `notices`, or
  setting either to `null`, means unsupported. Supplying `{}` advertises notice
  support. Agents must not send notices without this capability and may instead
  use an agent message when the information should still be surfaced to the
  user.
</ParamField>
...
<Card
  icon="message"
  horizontal
  href="/protocol/v1/draft/prompt-turn#session-notices"
>
...
<ResponseField name="loadSession" type="boolean" post={["default: false"]}>
  The [`session/load`](/protocol/v1/draft/session-setup#loading-sessions) method
  is available.
</ResponseField>

<ResponseField name="promptCapabilities" type="PromptCapabilities Object">
  Object indicating the different types of [content](/protocol/v1/draft/content)
...
Optionally, they **MAY** support richer types of [content](/protocol/v1/draft/content) by specifying the following capabilities:
...
<ResponseField name="logout" type="LogoutCapabilities Object">
  The [`logout`](/protocol/v1/draft/authentication#logging-out) method is
  available.
</ResponseField>

<Card icon="shield-check" horizontal href="/protocol/v1/draft/authentication">
...
Once the connection is initialized, you're ready to [create a session](/protocol/v1/draft/session-setup) and begin the conversation with the Agent.
