...
Agents can advertise a set of slash commands that users can invoke. These commands provide quick access to specific agent capabilities and workflows. Commands are run as part of regular [prompt](/protocol/v1/draft/prompt-turn) requests where the Client includes the command text in the prompt.
