...
- User prompts sent via [`session/prompt`](/protocol/v1/draft/prompt-turn#1-user-message)
- Language model output streamed through [`session/update`](/protocol/v1/draft/prompt-turn#3-agent-reports-output) notifications
- Progress updates and results from [tool calls](/protocol/v1/draft/tool-calls)
...
capability](/protocol/v1/draft/initialization#prompt-capabilities) when included
in prompts.
