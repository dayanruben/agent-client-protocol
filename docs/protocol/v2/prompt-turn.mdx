...
A prompt turn represents a complete interaction cycle between the [Client](/protocol/v2/overview#client) and [Agent](/protocol/v2/overview#agent), starting with a user message and continuing until the Agent completes its response. This may involve multiple exchanges with the language model and tool invocations.
