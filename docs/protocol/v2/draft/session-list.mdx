...
Before listing sessions, Clients **MUST** first complete the [initialization](/protocol/v2/draft/initialization) phase to verify the Agent supports this capability.
...
<ResponseField name="_meta" type="object">
  Agent-specific metadata. See
  [Extensibility](/protocol/v2/draft/extensibility).
</ResponseField>
...
3. Client calls [`session/load`](/protocol/v2/draft/session-setup#loading-sessions) with the chosen `sessionId` to resume the conversation
