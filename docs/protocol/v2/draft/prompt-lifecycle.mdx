...
A prompt starts or contributes to active work in a session. The [Agent](/protocol/v2/draft/overview#agent) may continue processing until it reports that the session is idle again, and it may emit `session/update` notifications before or after an individual prompt request has completed. Active work can involve multiple exchanges with the language model and tool invocations.
