...
Live-only `notice` updates are not conversation history and **SHOULD NOT** be
replayed. If the underlying condition is still relevant after resuming
finishes, the Agent may emit a new notice as a new live event.
