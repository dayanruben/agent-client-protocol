...
- Client: completes an advertised authentication flow if required; for example,
  `auth/login` for an Agent-handled method or launch and reconnect for a
  terminal method
...
<Step title="Prompt Lifecycle">
   ...
   - Agent → Client: `session/update` notifications for accepted messages, state updates, progress updates, and advisory notices
...
<ResponseField
  name="auth/login"
  post={[<a href="/protocol/v2/draft/schema#auth%2Flogin">Schema</a>]}
>
...
<ResponseField
  name="auth/logout"
  post={[<a href="/protocol/v2/draft/schema#auth%2Flogout">Schema</a>]}
>
  [End the current authenticated
  state](/protocol/v2/draft/authentication#logging-out).
</ResponseField>
...
>
  [Send user prompts](/protocol/v2/draft/prompt-lifecycle#1-user-message) to the
  Agent.
</ResponseField>

<ResponseField
  name="session/list"
  post={[<a href="/protocol/v2/draft/schema#session%2Flist">Schema</a>]}
>
  [List known sessions](/protocol/v2/draft/session-list).
</ResponseField>

<ResponseField
  name="session/resume"
  post={[<a href="/protocol/v2/draft/schema#session%2Fresume">Schema</a>]}
>
  [Resume an existing session, optionally replaying
  history](/protocol/v2/draft/session-setup#resuming-sessions).
</ResponseField>

<ResponseField
  name="session/close"
  post={[<a href="/protocol/v2/draft/schema#session%2Fclose">Schema</a>]}
>
  [Close an active
  session](/protocol/v2/draft/session-setup#closing-active-sessions).
</ResponseField>
...
>
  [Cancel ongoing operations](/protocol/v2/draft/prompt-lifecycle#cancellation)
...
>
  [Request user
  authorization](/protocol/v2/draft/tool-calls#requesting-permission) for
...
<ResponseField
  ...
  post={[<a href="/protocol/v2/draft/schema#elicitation%2Fcreate">Schema</a>]}
>
  [Request structured information from the user](/protocol/v2/draft/elicitation)
...
<ResponseField
  ...
  post={[<a href="/protocol/v2/draft/schema#elicitation%2Fcomplete">Schema</a>]}
>
  [Report completion of an out-of-band URL
  interaction](/protocol/v2/draft/elicitation#url-completion) (no response
  expected).
</ResponseField>
...
>
  [Send session
  updates](/protocol/v2/draft/prompt-lifecycle#3-agent-reports-output) to inform
  the Client of changes (no response expected). This includes [message updates
  and chunks](/protocol/v2/draft/content), [tool calls, updates, and content
  chunks](/protocol/v2/draft/tool-calls), [display-only terminal state and
  output chunks](/protocol/v2/draft/tool-calls#display-only-terminals),
  ...
  option updates](/protocol/v2/draft/session-config-options#from-the-agent), and
  [advisory notices](/protocol/v2/draft/prompt-lifecycle#session-notices).
</ResponseField>
...
- Review the [Prompt Lifecycle](/protocol/v2/draft/prompt-lifecycle)
