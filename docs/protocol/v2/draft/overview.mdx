...
- Client → Agent: `session/load` or `session/resume` to resume an existing session
...
>
  [Resume an existing session without replaying
...
>
  ...
  chunks](/protocol/v2/draft/tool-calls),
