...
ACP authentication is negotiated during [initialization](/protocol/v2/initialization). Agents advertise available authentication methods in `authMethods`. Clients authenticate by calling `auth/login` and end the current authenticated state by calling `auth/logout`.
