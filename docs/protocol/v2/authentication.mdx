...
ACP authentication is negotiated during [initialization](/protocol/v2/initialization). Agents advertise available authentication methods in `authMethods`, Clients choose one by calling `auth/login`, and Agents that support ending an authenticated state advertise the `logout` auth capability.
