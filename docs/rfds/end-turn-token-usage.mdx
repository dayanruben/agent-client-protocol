...
The current strawman is to add an optional `usage` field to `PromptResponse`, returned when a prompt turn completes:
...
3. **Streaming semantics** - Should partial usage be allowed during streaming, or should the protocol only report final usage on `PromptResponse`?
