...
This RFD does not add end-turn token usage to `PromptResponse`. That proposal remains in draft in the [End-Turn Token Usage RFD](/rfds/end-turn-token-usage).
...
**Everything in `PromptResponse`** - Simpler, but context window and cost are session state that users may want independently of prompt turns.
